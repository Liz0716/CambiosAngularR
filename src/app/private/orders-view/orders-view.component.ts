...
async ngOnInit() {
  ...
  if (JSON.parse(localStorage.getItem('user')|| '').rol == 3) {
    ...
    let params = {
      users_idusers: JSON.parse(localStorage.getItem('user')|| '{}').idusers
    }
    console.log(params);

    this.order = await this._provider.request('GET', 'order/viewOrdersId/',params);
  }
  else{
